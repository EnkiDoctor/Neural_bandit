...
def get_nonlinear_purchase(feature_list):
    true_Vlist = [(TRUE_THETA @ relu(feature_list[i]).reshape(-1,1)).item() for  i in range(len(feature_list))]
...
def calculate_relu_v(contextinfo_list, A, theta):
    vj_list = []
    feature_list = []
    for i in contextinfo_list:
        feature = relu(i)
...
def get_true_relu_ass(context, profit):
    
    v_array = np.array(relu(context) @ TRUE_THETA.T ).reshape(-1)
    assortment, ass_features, v_list, profit_list = assort(context, profit.tolist()[0], v_array.tolist() , relu(context))
...
def get_assortment_revenue(assortment, profit):
    v_array = np.array(relu(np.array(assortment)) @ TRUE_THETA.T ).reshape(-1)
...
import modeldefine
import torch.optim as optim
model = modeldefine.Model(5,10,10,3).to(device)
# 10 20 20 20 20 20  5
optimizer = optim.Adam(model.parameters(), lr=0.001)

# data reader
CONTEXT_ARRAY = np.load('relu_data/features.npy') 
REWARD_ARRAY = np.load('relu_data/rewards.npy')
TRUE_THETA = np.load('relu_data/theta.npy')
...
for t in range(0, 15000):
    ...
    # calculate the ideal 
    expected_revenue1 = get_true_relu_ass(context, profit)
...
np.save('relu_data/record/15true_revenue_list1.npy', np.array(revenue_list1))
np.save('relu_data/record/15simulate_revenue_list2.npy', np.array(revenue_list2))
np.save('relu_data/record/15purchase_revenue_list.npy', np.array(true_profit_list))